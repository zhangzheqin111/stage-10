...
- 阶段 4-1 已完成：摄像头授权引导、实时预览、摄像头 / 触摸模式显示、启动失败触摸兜底、摄像头请求降级重试；尚未接入 MediaPipe Hands 手势分类。
...
- 刷新选歌页、内容页、预览页时，会清空上一次上传的音频和图片资源。
...
- 阶段 4-1 只完成摄像头授权、预览和兜底，还未接入 MediaPipe Hands 手势识别。
- 摄像头唤起稳定性仍受浏览器站点权限、HTTPS / localhost 安全上下文、摄像头设备占用和移动浏览器策略影响。
...
- 接入 MediaPipe Hands。
- 将摄像头识别到的 4 类基础手势映射到数据面板和花园交互：
  - 手掌上下摆动 -> 高度。
  - 手掌左右摇晃 -> 风力 / 左右摇晃。
  - 张开五指并拢双拳 -> 开合。
  - 拇指与食指孔雀形状捏合 -> 换色。
- 真机验证摄像头授权、拒绝授权和触摸兜底。
- 已完成：摄像头授权提示、失败自动进入触摸模式、摄像头画面不保存不上传、实时预览、数据面板显示摄像头 / 触摸模式、摄像头请求降级重试。
...
后续 MediaPipe Hands 应接入当前 `cameraStreamRef` / video 输入，不改变 Gift 数据结构，不上传摄像头画面。
...
1. 若继续阶段 4，优先接 MediaPipe Hands，只实现基础手势识别和映射，不做阶段 5 视觉精修。
2. 若回补阶段 2，优先配置 Supabase 并做跨设备分享验收。
3. 若继续阶段 3，优先接真实音乐 API 数据源或系统 BGM 编辑，不重写页面。
4. 阶段 5 再做全站视觉和移动端细节打磨。
