...
- 阶段 5 当前已开始并完成两个明确模块：分享链接复制反馈优化、预览页 / 礼物页加载状态补齐与最短 720ms 可见提示。移动端布局、失败态、空状态、动画自然度和主题视觉精修仍待继续逐项处理。
...
- 摄像头唤起稳定性仍受浏览器站点权限、HTTPS / localhost 安全上下文、摄像头设备占用和移动浏览器策略影响；两段式启动只能减少模型加载造成的等待感。
...
1. 若继续阶段 5，优先处理移动端布局溢出与小屏文本适配，检查分享弹窗、底部按钮、数据面板、引导卡和礼物舞台在窄屏下是否遮挡或横向滚动。
2. 若回补阶段 2，优先配置 Supabase 并做跨设备分享验收。
3. 若继续阶段 3，优先接真实音乐 API 数据源或系统 BGM 编辑，不重写页面。
4. 若继续阶段 4，优先做真机摄像头手势调参，先验收张开 / 握拳、初始高度基准、左右 / 上下串扰、捏合换色和摄像头 / 触摸切换。
