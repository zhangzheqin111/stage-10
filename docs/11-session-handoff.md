...
- 阶段 4 当前已完成：摄像头授权引导、实时预览、摄像头 / 触摸模式显示、启动失败触摸兜底、摄像头请求降级重试、MediaPipe HandLandmarker 基础识别与花园交互映射、双手同向融合、捏合状态机、摄像头 / 触摸手动切换、手势灵敏度止血修复；仍需真机调参。
...
1. 若继续阶段 4，优先做真机摄像头手势调参，先验收张开 / 握拳、初始高度基准、左右 / 上下串扰、捏合换色和摄像头 / 触摸切换，不做阶段 5 视觉精修。
